...
class ReDirectSTD(object):
    ...
    def write(self, msg):
        ...
        if self.file is not None:
            if not os.path.exists(os.path.dirname(os.path.abspath(self.file.replace(':', '.')))):
                os.mkdir(os.path.dirname(os.path.abspath(self.file.replace(':', '.'))))

            if self.immediately_visiable:
                # open for writing, appending to the end of the file if it exists
                with open(self.file.replace(':', '.'), 'a') as f:
                    f.write(msg)
            else:
                if self.f is None:
                    self.f = open(self.file.replace(':', '.'), 'w')
