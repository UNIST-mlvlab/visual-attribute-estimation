...
def get_pedestrian_metrics(gt_label, preds_probs, threshold=0.5, index=None):
    ...
    #result.label_acc = true_pos + true_neg / (true_pos + true_neg + false_pos + false_neg + eps)
    ...
    result.ins_instance_prec = instance_prec
    result.ins_instance_recall = instance_recall
    
    mean_instance_acc = np.mean(instance_acc)
    mean_instance_prec = np.mean(instance_prec)
    mean_instance_recall = np.mean(instance_recall)
    # instance_f1 = np.mean(instance_f1)
    mean_instance_f1 = 2 * mean_instance_prec * mean_instance_recall / (mean_instance_prec + mean_instance_recall + eps)

    result.instance_acc = mean_instance_acc
    result.instance_prec = mean_instance_prec
    result.instance_recall = mean_instance_recall
    result.instance_f1 = mean_instance_f1
